...
// src/app.ts
var app = express();
var STATIC_PATH = path.join(process.cwd(), "packages/client/dist/public");
...
app.get("/debug/paths", (_req, res) => {
  ...
  try {
    ...
    results["cwd"] = "unavailable";
  }
  try {
    results["/app"] = fs.readdirSync("/app").slice(0, 10);
  } catch (e) {
    results["/app"] = e.code;
  }
  try {
    results["~"] = fs.readdirSync(process.env.HOME || "/").slice(0, 10);
  } catch (e) {
    results["~"] = e.code;
  }
  res.json(results);
...
var indexFile = String(path.join(STATIC_PATH, "index.html"));
app.get("/", (_req, res) => {
